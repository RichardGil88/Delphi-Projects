object FchequeosKD: TFchequeosKD
  Left = 0
  Top = 0
  Caption = 'Chequeos  KD'
  ClientHeight = 716
  ClientWidth = 920
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object GB_CHEQUEOSKD: TcxGroupBox
    Left = 0
    Top = 0
    Align = alClient
    Caption = 'CHEQUEOS'
    TabOrder = 0
    Height = 716
    Width = 920
    object cxGroupBox1: TcxGroupBox
      Left = 3
      Top = 15
      Align = alTop
      Caption = 'AGREGAR'
      TabOrder = 0
      Height = 279
      Width = 914
      object button1: TcxButton
        Left = 24
        Top = 234
        Width = 90
        Height = 33
        Caption = 'AGREGAR'
        Glyph.Data = {
          }
        TabOrder = 18
        OnClick = button1Click
      end
      object button2: TcxButton
        Left = 767
        Top = 234
        Width = 89
        Height = 33
        Caption = 'CANCELAR'
        Glyph.Data = {
          
          000000000000000000000101676909097D7F0000000000000000}
        TabOrder = 19
        OnClick = button2Click
      end
      object cxRadioGroup2: TcxRadioGroup
        Left = 167
        Top = 152
        Caption = 'Cumplimiento'
        Properties.Items = <>
        TabOrder = 20
        Height = 87
        Width = 121
      end
      object cxRadioButton3: TcxRadioButton
        Left = 185
        Top = 183
        Width = 61
        Height = 16
        Caption = 'Si'
        TabOrder = 15
        OnClick = cxRadioButton3Click
        Transparent = True
      end
      object cxRadioButton4: TcxRadioButton
        Left = 185
        Top = 212
        Width = 61
        Height = 16
        Caption = 'No'
        TabOrder = 16
        OnClick = cxRadioButton4Click
        Transparent = True
      end
      object cxLookupComboBox3: TcxLookupComboBox
        Left = 310
        Top = 160
        ParentShowHint = False
        Properties.KeyFieldNames = 'motivoNoCumplimiento'
        Properties.ListColumns = <
          item
            FieldName = 'motivoNoCumplimiento'
          end>
        Properties.ListSource = DM.DSmotcump
        ShowHint = True
        TabOrder = 17
        Width = 371
      end
      object cxLabel10: TcxLabel
        Left = 310
        Top = 146
        Caption = 'Moivo del no cumplimiento'
        Transparent = True
      end
      object cxMaskEdit1: TcxMaskEdit
        Left = 24
        Top = 32
        Properties.MaskKind = emkRegExpr
        Properties.EditMask = '\d+'
        Properties.MaxLength = 0
        TabOrder = 0
        Width = 121
      end
      object cxLabel1: TcxLabel
        Left = 24
        Top = 16
        Caption = 'Clave *'
        Transparent = True
      end
      object cxTextEdit1: TcxTextEdit
        Left = 280
        Top = 32
        TabOrder = 2
        Width = 121
      end
      object cxLabel2: TcxLabel
        Left = 280
        Top = 16
        Caption = 'Caso *'
        Transparent = True
      end
      object cxLookupComboBox1: TcxLookupComboBox
        Left = 407
        Top = 32
        Properties.KeyFieldNames = 'LUGARES'
        Properties.ListColumns = <
          item
            FieldName = 'LUGARES'
          end>
        Properties.ListSource = DM.DSlugmon
        TabOrder = 3
        Width = 145
      end
      object cxLabel3: TcxLabel
        Left = 407
        Top = 16
        Caption = 'Lugar de Monitoreo *'
        Transparent = True
      end
      object cxLookupComboBox2: TcxLookupComboBox
        Left = 560
        Top = 32
        Properties.KeyFieldNames = 'Tipo'
        Properties.ListColumns = <
          item
            FieldName = 'Tipo'
          end>
        Properties.ListSource = DM.DSTtipochequeo
        TabOrder = 4
        Width = 145
      end
      object cxLabel4: TcxLabel
        Left = 560
        Top = 16
        Caption = 'Tipo de Chequeo *'
        Transparent = True
      end
      object cxLookupComboBox4: TcxLookupComboBox
        Left = 711
        Top = 32
        Properties.KeyFieldNames = 'objeto de aplicacion'
        Properties.ListColumns = <
          item
            FieldName = 'objeto de aplicacion'
          end>
        Properties.ListSource = DM.DSTobjetoaplicacion
        TabOrder = 5
        Width = 145
      end
      object cxLabel5: TcxLabel
        Left = 712
        Top = 16
        Caption = 'Objeto de aplicaci'#243'n *'
        Transparent = True
      end
      object cxTextEdit2: TcxTextEdit
        Left = 151
        Top = 32
        TabOrder = 1
        Width = 121
      end
      object cxLabel6: TcxLabel
        Left = 151
        Top = 16
        Caption = 'Chequeo *'
        Transparent = True
      end
      object cxTextEdit3: TcxTextEdit
        Left = 24
        Top = 75
        TabOrder = 6
        Width = 121
      end
      object cxLabel7: TcxLabel
        Left = 24
        Top = 59
        Caption = 'Objetivo *'
        Transparent = True
      end
      object cxLabel8: TcxLabel
        Left = 151
        Top = 59
        Caption = 'Fase *'
        Transparent = True
      end
      object cxLookupComboBox5: TcxLookupComboBox
        Left = 151
        Top = 74
        ParentShowHint = False
        Properties.KeyFieldNames = 'fase'
        Properties.ListColumns = <
          item
            FieldName = 'fase'
          end>
        Properties.ListSource = DM.DSfase
        ShowHint = True
        TabOrder = 7
        Width = 401
      end
      object cxLabel9: TcxLabel
        Left = 560
        Top = 58
        Caption = 'Fecha Alta *'
        Transparent = True
      end
      object cxDateEdit1: TcxDateEdit
        Left = 560
        Top = 74
        Properties.Kind = ckDateTime
        TabOrder = 8
        Width = 146
      end
      object cxDateEdit2: TcxDateEdit
        Left = 712
        Top = 78
        Properties.Kind = ckDateTime
        TabOrder = 9
        Width = 144
      end
      object cxLabel11: TcxLabel
        Left = 711
        Top = 59
        Caption = 'Fecha Baja '
        Transparent = True
      end
      object cxTextEdit4: TcxTextEdit
        Left = 24
        Top = 120
        TabOrder = 10
        Width = 377
      end
      object cxLabel12: TcxLabel
        Left = 24
        Top = 104
        Caption = 'Intereses *'
        Transparent = True
      end
      object cxLookupComboBox6: TcxLookupComboBox
        Left = 407
        Top = 120
        Properties.KeyFieldNames = 'dptoseccion'
        Properties.ListColumns = <
          item
            FieldName = 'dptoseccion'
          end>
        Properties.ListSource = DM.DSdptosysecc
        TabOrder = 11
        Width = 145
      end
      object cxLabel13: TcxLabel
        Left = 407
        Top = 104
        Caption = #211'rgano *'
        Transparent = True
      end
      object cxTextEdit5: TcxTextEdit
        Left = 560
        Top = 120
        TabOrder = 12
        Width = 146
      end
      object cxLabel14: TcxLabel
        Left = 560
        Top = 104
        Caption = 'Oficial *'
        Transparent = True
      end
      object cxLabel15: TcxLabel
        Left = 711
        Top = 105
        Caption = 'Nivel de Aprobaci'#243'n *'
        Transparent = True
      end
      object cxLookupComboBox7: TcxLookupComboBox
        Left = 712
        Top = 120
        ParentShowHint = False
        Properties.KeyFieldNames = 'Nivel Aprobaci'#243'n'
        Properties.ListColumns = <
          item
            FieldName = 'Nivel Aprobaci'#243'n'
          end>
        Properties.ListSource = DM.DSniveldelAprobacion
        ShowHint = True
        TabOrder = 13
        Width = 144
      end
      object cxLookupComboBox8: TcxLookupComboBox
        Left = 24
        Top = 160
        Properties.KeyFieldNames = 'niveldecompartimentacion'
        Properties.ListColumns = <
          item
            FieldName = 'niveldecompartimentacion'
          end>
        Properties.ListSource = DM.DSTnivelcompart
        TabOrder = 14
        Width = 121
      end
      object cxLabel16: TcxLabel
        Left = 24
        Top = 146
        Caption = 'Compartimentaci'#243'n *'
        Transparent = True
      end
    end
    object cxGroupBox2: TcxGroupBox
      Left = 3
      Top = 294
      Align = alClient
      Caption = 'TODOS'
      TabOrder = 1
      Height = 413
      Width = 914
      object AdvCircularProgress1: TAdvCircularProgress
        Left = 424
        Top = 168
        Width = 60
        Height = 60
        Appearance.BackGroundColor = clNone
        Appearance.BorderColor = clNone
        Appearance.ActiveSegmentColor = 16752543
        Appearance.InActiveSegmentColor = clSilver
        Appearance.TransitionSegmentColor = 10485760
        Appearance.ProgressSegmentColor = 4194432
        Interval = 100
      end
      object cxGrid1: TcxGrid
        Left = 3
        Top = 15
        Width = 908
        Height = 389
        Align = alClient
        PopupMenu = AdvPopupMenu1
        TabOrder = 0
        ExplicitLeft = 99
        ExplicitTop = 57
        object cxGrid1DBTableView1: TcxGridDBTableView
          NavigatorButtons.ConfirmDelete = False
          NavigatorButtons.PriorPage.Visible = True
          NavigatorButtons.Prior.Visible = True
          NavigatorButtons.Insert.Visible = True
          NavigatorButtons.Append.Visible = True
          NavigatorButtons.Delete.Visible = True
          NavigatorButtons.Edit.Visible = True
          NavigatorButtons.Post.Visible = True
          NavigatorButtons.Cancel.Visible = True
          NavigatorButtons.Refresh.Visible = True
          NavigatorButtons.SaveBookmark.Visible = True
          NavigatorButtons.GotoBookmark.Visible = True
          NavigatorButtons.Filter.Visible = True
          FilterBox.Visible = fvNever
          OnCellClick = cxGrid1DBTableView1CellClick
          DataController.DataModeController.SmartRefresh = True
          DataController.DataSource = DM.DSTchequeosKD
          DataController.Summary.DefaultGroupSummaryItems = <
            item
              Format = 'CANTIDAD=#'
              Kind = skCount
              FieldName = 'CLAVES'
            end>
          DataController.Summary.FooterSummaryItems = <
            item
              Kind = skCount
              FieldName = 'IDCHEQUEOSKD'
              Column = cxGrid1DBTableView1UNIDAD
            end>
          DataController.Summary.SummaryGroups = <>
          DateTimeHandling.Filters = [dtfRelativeDays, dtfRelativeDayPeriods, dtfRelativeYears, dtfMonths, dtfYears]
          Filtering.ColumnFilteredItemsList = True
          FilterRow.Visible = True
          OptionsBehavior.IncSearch = True
          OptionsBehavior.NavigatorHints = True
          OptionsCustomize.ColumnHidingOnGrouping = False
          OptionsCustomize.GroupRowSizing = True
          OptionsData.Appending = True
          OptionsSelection.CellMultiSelect = True
          OptionsView.Navigator = True
          OptionsView.CellAutoHeight = True
          OptionsView.Footer = True
          OptionsView.Indicator = True
          OptionsView.IndicatorWidth = 15
          object cxGrid1DBTableView1UNIDAD: TcxGridDBColumn
            DataBinding.FieldName = 'UNIDAD'
            Width = 93
          end
          object cxGrid1DBTableView1CLAVE: TcxGridDBColumn
            DataBinding.FieldName = 'CLAVE'
            Width = 109
          end
          object cxGrid1DBTableView1CASO: TcxGridDBColumn
            DataBinding.FieldName = 'CASO'
            Width = 195
          end
          object cxGrid1DBTableView1ESTADO: TcxGridDBColumn
            DataBinding.FieldName = 'ESTADO'
          end
          object cxGrid1DBTableView1LMONITOREO: TcxGridDBColumn
            DataBinding.FieldName = 'L/MONITOREO'
            Width = 162
          end
          object cxGrid1DBTableView1FECHAALTA: TcxGridDBColumn
            DataBinding.FieldName = 'FECHA ALTA'
          end
          object cxGrid1DBTableView1ORGANODPTO: TcxGridDBColumn
            DataBinding.FieldName = 'ORGANO/DPTO'
          end
          object cxGrid1DBTableView1OFICIAL: TcxGridDBColumn
            DataBinding.FieldName = 'OFICIAL'
          end
          object cxGrid1DBTableView1NIVELDEAPROBACION: TcxGridDBColumn
            DataBinding.FieldName = 'NIVEL DE APROBACION'
            Width = 119
          end
          object cxGrid1DBTableView1TIPODECHEQUEO: TcxGridDBColumn
            DataBinding.FieldName = 'TIPO DE CHEQUEO'
          end
          object cxGrid1DBTableView1OBJETODEAPLICACION: TcxGridDBColumn
            DataBinding.FieldName = 'OBJETO DE APLICACION'
          end
          object cxGrid1DBTableView1CHEQUEO: TcxGridDBColumn
            DataBinding.FieldName = 'CHEQUEO'
          end
          object cxGrid1DBTableView1OBJETIVO: TcxGridDBColumn
            DataBinding.FieldName = 'OBJETIVO'
          end
          object cxGrid1DBTableView1FASE: TcxGridDBColumn
            DataBinding.FieldName = 'FASE'
          end
          object cxGrid1DBTableView1INTERESES: TcxGridDBColumn
            DataBinding.FieldName = 'INTERESES'
          end
          object cxGrid1DBTableView1COMPARTIMENTACION: TcxGridDBColumn
            DataBinding.FieldName = 'COMPARTIMENTACION'
            Width = 118
          end
          object cxGrid1DBTableView1FECHABAJA: TcxGridDBColumn
            DataBinding.FieldName = 'FECHA BAJA'
          end
          object cxGrid1DBTableView1CUMPLIMIENTOFINALIDAD: TcxGridDBColumn
            DataBinding.FieldName = 'CUMPLIMIENTO FINALIDAD'
            Width = 138
          end
          object cxGrid1DBTableView1MOTIVOSNOCUMPLIMIENTO: TcxGridDBColumn
            DataBinding.FieldName = 'MOTIVOS NO CUMPLIMIENTO'
            Width = 506
          end
          object cxGrid1DBTableView1USUARIO: TcxGridDBColumn
            DataBinding.FieldName = 'USUARIO'
            Width = 89
          end
          object cxGrid1DBTableView1IDCHEQUEOSKD: TcxGridDBColumn
            DataBinding.FieldName = 'IDCHEQUEOSKD'
          end
        end
        object cxGridLevel2: TcxGridLevel
          GridView = cxGrid1DBTableView1
        end
      end
      object cxButton1: TcxButton
        Left = 718
        Top = 18
        Width = 123
        Height = 33
        Caption = 'EXPORTAR A XLS'
        TabOrder = 1
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -11
        Font.Name = 'Arial Black'
        Font.Style = []
        ParentFont = False
        OnClick = cxButton1Click
      end
    end
  end
  object AdvPopupMenu1: TAdvPopupMenu
    Version = '2.5.3.1'
    Left = 848
    Top = 184
    object DarBaja1: TMenuItem
      Caption = 'Dar Baja'
      OnClick = DarBaja1Click
    end
  end
end
